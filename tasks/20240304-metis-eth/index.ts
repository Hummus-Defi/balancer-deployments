...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  if (task.network === 'metisGoerli') {
    rateProviders = [input.ETH_PROVIDER, input.METIS_PROVIDER];
  ...
  await task.save({ MetisWethPool: poolAddress });

  const pool = await task.instanceAt('WeightedPool', task.output()['MetisWethPool']);
