...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  if (task.network === 'metisGoerli') {
    rateProviders = [input.SFRXETH_PROVIDER, input.ETH_PROVIDER];
  ...
  const newWeightedPoolParams = {
    name: 'Frax Feast',
    symbol: 'FEAST',
    tokens: [input.SFRXETH, input.ETH].sort(function (a, b) {
  ...
  await task.save({ FraxFeastPool: poolAddress });

  const pool = await task.instanceAt('WeightedPool', task.output()['FraxFeastPool']);
